...
export default function InfluenciadorPublicPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F0E1A] text-white flex flex-col justify-between selection:bg-[#5B50E5] selection:text-white">
      <div>
        {/* HERO CONVIDATIVA & MOTIVACIONAL HORIZONTAL */}
        <div
          className="relative overflow-hidden py-10 px-6 sm:px-12"
          style={{
        ...
        >
          ...
          <div
            className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-64 blur-[100px] opacity-35 pointer-events-none"
          ...
          <div className="max-w-4xl mx-auto relative z-10">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
              ...
              <div className="flex-1 text-center md:text-left space-y-2.5 min-w-0">
                ...
                <p className="text-sm font-semibold text-[#A78BFA] flex items-center justify-center md:justify-start gap-2">
        ...
        {/* Conteúdo Principal Ampliado (max-w-4xl) */}
        <div className="max-w-4xl mx-auto px-6 py-8 space-y-8">
          ...
          <div className="space-y-3.5">
            <button
              ...
              className={`w-full py-4 sm:py-5 px-6 rounded-2xl font-extrabold text-sm sm:text-base flex items-center justify-center gap-3 transition-all transform active:scale-[0.99] shadow-2xl ${
                copied
          ...
          {/* PRÉVIA INFORMATIVA EM GRID HORIZONTAL (3 COLUNAS NO DESKTOP) */}
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {/* Coluna 1: Links UTM */}
              <div
                className="rounded-2xl p-5 space-y-3 flex flex-col justify-between"
                  ...
                  <div className="space-y-2">
                    {utmLinks.map((u) => (
                      <div key={u.plataforma} className="flex items-center justify-between text-xs py-1.5 px-3 rounded-xl bg-white/5 font-mono text-white/80">
                        ...
                        <span className="truncate max-w-[120px] text-white/40 text-[11px]">{u.urlCompleta}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5 text-xs font-mono text-white/70">
                    <div className="flex items-center justify-between py-1.5 px-3 rounded-xl bg-white/5">
                      ...
                      <span className="truncate max-w-[130px] text-white/50">{raizBio}</span>
                    </div>
                    {arvoreLinks.map((s) => (
                      <div key={s.plataforma} className="flex items-center justify-between py-1 px-3 text-[11px]">
                        <span className="text-white/60">{s.label}:</span>
                        <span className="truncate max-w-[120px] text-white/40">{s.urlCompleta}</span>
                      </div>
                    ))}
                  </div>
                </div>
